...
public class DragRotateY : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            float mouseY = Input.GetAxis("Mouse Y"); // Get horizontal mouse movement
            currentRotation += mouseY * rotationSpeed; // Accumulate rotation based on mouse movement

            // Clamp the rotation to the defined limits
            currentRotation = Mathf.Clamp(currentRotation, minRotation, maxRotation);

            // Apply the rotation to the turntable object
            transform.rotation = Quaternion.Euler(currentRotation, 0, 0); // Rotate around Y-axis
        }
    }
}
